...
dds_hc <- DESeqDataSetFromMatrix(countData = rna,
                              colData = clinical,
                              design = ~ definition)

#dds for alive/dead comparison
##remove non reported cases from clinical df
keep = !clinical$vital_status=="Not Reported" & clinical$definition=="Primary_solid_Tumor"
clinical_ad = clinical[keep,]
clinical_ad$vital_status <- as.factor(clinical_ad$vital_status)
levels(clinical_ad$vital_status)
clinical_ad$vital_status <- relevel(clinical_ad$vital_status, ref = "Alive")

##remove non reported cases from rna df
rna_ad = rna[,rownames(clinical[keep,])]

dds_ad <- DESeqDataSetFromMatrix(countData = rna_ad,
                                 colData = clinical_ad,
                                 design = ~ vital_status)

# prefilteration: it is not necessary but recommended to filter out low expressed genes
keep <- rowSums(counts(dds_hc)) >= 10
dds_hc <- dds_hc[keep,]

# prefilteration for aive/healthy dds: it is not necessary but recommended to filter out low expressed genes
keep <- rowSums(counts(dds_ad)) >= 10
dds_ad <- dds_ad[keep,]
...
##specify dds object (healthy/cancer or alive/dead comparison)
dds = dds_ad
